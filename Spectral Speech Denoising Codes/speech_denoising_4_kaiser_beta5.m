...
window=kaiser(window_length, 5); %create a hamming window in length of window_length. the input signal x will be divided into segments in length of window variable.
...
shading interp;
colormap('jet');
title('Kaiser Windowed Spectrogram (\beta = 5): speech + audience');
xlabel('Time (s)');
ylabel('Frequency (Hz)');
...
shading interp;
colormap('jet');
title('Kaiser Windowed Spectrogram (\beta = 5): speech only');
xlabel('Time (s)');
ylabel('Frequency (Hz)');
